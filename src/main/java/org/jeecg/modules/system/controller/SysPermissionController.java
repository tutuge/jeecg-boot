    ...
    public Result<?> getUserPermissionByToken(HttpServletRequest request) {
        ...
        try {
            ...
            if (!PermissionDataUtil.hasIndexPage(metaList)) {
                List<SysPermission> sysPermissions = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "报价管理"));
                SysPermission indexMenu = sysPermissions.get(0);
            ...
            if (roleIndex != null) {
                List<SysPermission> menus = metaList.stream().filter(sysPermission -> "报价管理".equals(sysPermission.getName())).collect(Collectors.toList());
