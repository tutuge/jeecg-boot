...
public class EcOfferModel {
    ...
    private String convertOffer(EcOfferInsertBo bo) {
        //后台查询判断材料名称
        //导体
        Conductor conductor = bo.getConductor();
        EcbMaterials objectPassId = ecbMaterialsModel.getObjectPassId(conductor.getId());
        conductor.setFullName(objectPassId.getFullName());
        //填充物
        Infilling infilling = bo.getInfilling();
        EcbMaterials objectPassId0 = ecbMaterialsModel.getObjectPassId(infilling.getId());
        infilling.setFullName(objectPassId0.getFullName());
        //内部材料
        List<Internal> internals = bo.getInternals();
        for (Internal internal : internals) {
            EcbMaterials objectPassId1 = ecbMaterialsModel.getObjectPassId(internal.getId());
            internal.setFullName(objectPassId1.getFullName());
        }
        //外部材料
        List<External> externals = bo.getExternals();
        for (External external : externals) {
            EcbMaterials objectPassId1 = ecbMaterialsModel.getObjectPassId(external.getId());
