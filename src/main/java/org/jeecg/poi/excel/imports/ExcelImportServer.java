 ...
 */
package org.jeecg.poi.excel.imports;
...
import org.jeecg.poi.excel.entity.params.ExcelCollectionParams;
import org.jeecg.poi.excel.entity.params.ExcelImportEntity;
...
import org.jeecg.poi.excel.imports.base.ImportBaseService;
import org.jeecg.poi.excel.imports.base.ImportFileServiceI;
import org.jeecg.poi.excel.imports.verifys.VerifyHandlerServer;
import org.jeecg.poi.exception.excel.ExcelImportException;
import org.jeecg.poi.exception.excel.enums.ExcelImportEnum;
import org.jeecg.poi.util.ExcelUtil;
import org.jeecg.poi.util.PoiPublicUtil;
import org.jeecg.util.ApplicationContextUtil;
...
//		}
        ...
        for (int i = params.getStartSheetIndex(); i < params.getStartSheetIndex()
            ...
            if (isXSSFWorkbook) {
                pictures = PoiPublicUtil.getSheetPictrues07((XSSFSheet) sheetAt, (XSSFWorkbook) book);
            } else {
                pictures = PoiPublicUtil.getSheetPictrues03((HSSFSheet) sheetAt, (HSSFWorkbook) book);
            }
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(" end to read excel by is ,endTime is {}", new Date().getTime());
            }
            List<T> objects = importExcel(sheetAt, pojoClass, params, pictures);
            result.addAll(objects);
            List<T> list = new ArrayList<>();
            list.addAll(objects);
            resultArray.add(list);
            sheetNames.add(sheetAt.getSheetName());
            if (LOGGER.isDebugEnabled()) {
        ...
        return new ExcelImportResult(result, resultArray, sheetNames, verfiyFail, book);
    }
