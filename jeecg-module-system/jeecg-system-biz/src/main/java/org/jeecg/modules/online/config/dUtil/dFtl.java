...
public class dFtl {
    private static final Logger a = LoggerFactory.getLogger(dFtl.class);
    ...
    public dFtl(bDataBaseConfig paramb) throws SQLException, AException {
        c = eDbTableHandle.a(paramb);
    }

    public dFtl() throws SQLException, AException {
        c = eDbTableHandle.a((bDataBaseConfig) null);
    }
    ...
    public static void a(Attribute parama) throws IOException, TemplateException, HibernateException, SQLException, AException {
        ...
        bDataBaseConfig b = parama.getDbConfig();
        if (d == null) {
        ...
        for (Exception exception : list) {
            if ("java.sql.SQLSyntaxErrorException".equals(exception.getCause().getClass().getName())) {
                ...
                if ("42000".equals(sQLSyntaxErrorException.getSQLState())) {
                    if (1064 == sQLSyntaxErrorException.getErrorCode() || 903 == sQLSyntaxErrorException.getErrorCode()) {
                        ...
                        throw new AException("请确认表名是否为关键字。");
                    }
                    continue;
                }
            } else {
                if ("com.microsoft.sqlserver.jdbc.SQLServerException".equals(exception.getCause().getClass().getName())) {
                    if (exception.getCause().toString().indexOf("Incorrect syntax near the keyword") != -1) {
                        exception.printStackTrace();
                        throw new AException(exception.getCause().getMessage());
                    }
            ...
            throw new AException(exception.getMessage());
        }
    }
    ...
    public List<String> b(Attribute parama) throws AException, SQLException {
        ...
        try {
            Map<String, aUtil> map1 = a(null, str2, parama.getDbConfig());
            Map<String, aUtil> map2 = this.c(parama);
            Map<String, String> map = a(parama.getColumns());
            for (String str : map2.keySet()) {
                if ("id".equalsIgnoreCase(str))
                    continue;
                if (!map1.containsKey(str)) {
                    aUtil a3 = map2.get(str);
                    String str4 = map.get(str);
                    if (map.containsKey(str) && map1.containsKey(str4)) {
                        aUtil a4 = map1.get(str4);
                ...
                aUtil a1 = map1.get(str);
                aUtil a2 = map2.get(str);
                if (DbType.DB2.equals(dbType) || DbType.HSQL.equals(dbType)) {
                    this.a(a1, a2, str2, arrayList);
                    continue;
                }
                if (!a1.a(a2, dbType))
    ...
    private static Map<String, Object> a(Attribute parama, DbType paramDbType) {
    ...
    private Map<String, aUtil> a(String paramString1, String paramString2, bDataBaseConfig paramb) throws SQLException {
        HashMap<Object, Object> hashMap = new HashMap<>(5);
        Connection connection = null;
        try {
            connection = eDbTableHandle.b(paramb);
        } catch (Exception exception) {
    ...
    private Map<String, aUtil> c(Attribute parama) {
        HashMap<Object, Object> hashMap = new HashMap<>(5);
        ...
        for (OnlCgformField onlCgformField : list) {
            ...
            a1.setColumnSize(onlCgformField.getDbLength());
            a1.setColunmType(onlCgformField.getDbType().toLowerCase());
            a1.setIsNullable((onlCgformField.getDbIsNull() == 1) ? "Y" : "N");
            a1.setComment(onlCgformField.getDbFieldTxt());
            a1.setDecimalDigits(onlCgformField.getDbPointLength());
    ...
    private String a(aUtil parama1, aUtil parama2) throws AException {
    ...
    private void a(aUtil parama1, aUtil parama2, String paramString, List<String> paramList) {
    ...
    public static List<String> a(String paramString) throws SQLException {
        ...
        ArrayList<String> arrayList = new ArrayList<>();
        try {
            connection = eDbTableHandle.getConnection();
            DatabaseMetaData databaseMetaData = connection.getMetaData();
